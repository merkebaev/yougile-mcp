...
const BASE_V2 = "https://ru.yougile.com/api-v2";
...
const server = new McpServer({ name: "yougile-mcp-server", version: "1.0.0" });
...
server.registerTool(
  ...
  async () => {
    const data = await apiv2("GET", "/projects?limit=100") as { content?: Array<{ id: string; title: string }> };
...
server.registerTool(
  ...
  async ({ projectId }) => {
    const data = await apiv2("GET", `/boards?projectId=${projectId}&limit=100`) as { content?: Array<{ id: string; title: string }> };
...
server.registerTool(
  ...
  async ({ boardId }) => {
    const data = await apiv2("GET", `/columns?boardId=${boardId}&limit=100`) as { content?: Array<{ id: string; title: string }> };
...
server.registerTool(
  "yougile_list_tasks",
  {
    ...
    description: "Получить список задач из конкретной колонки YouGile. Нужен columnId из yougile_list_columns.",
    inputSchema: z.object({
  ...
  async ({ columnId }) => {
    const data = await apiv2("GET", `/tasks?columnId=${columnId}&limit=100`) as { content?: Array<{ id: string; title: string; deadline?: string; assigned?: string[] }> };
    const tasks = data.content || [];
    if (!tasks.length) return text("Задачи не найдены");
    const list = tasks.map(t => {
      let row = `• ${t.title} [id: ${t.id}]`;
      if (t.deadline) row += ` | дедлайн: ${t.deadline}`;
      return row;
    }).join("\n");
...
server.registerTool(
  "yougile_create_task",
  {
    ...
    description: `Создать новую задачу в колонке YouGile.
Требует columnId (из yougile_list_columns) и title.
Приоритет задаётся через stringStickers: Важно | Нормально | Не важно.
Дедлайн в формате YYYY-MM-DD.`,
    inputSchema: z.object({
      ...
      assignee: z.string().optional().describe("ID пользователя для назначения (из yougile_list_users)")
    }).strict(),
  ...
  async ({ columnId, title, description, priority, deadline, assignee }) => {
    ...
    if (assignee) body.assigned = assignee;

    const data = await apiv1("POST", "/tasks", body) as { result: string; id?: string; error?: string };
    if (data.result === "ok") {
...
server.registerTool(
  "yougile_update_task",
  {
    ...
    description: "Обновить существующую задачу в YouGile. Нужен taskId. Можно менять название, описание, приоритет, дедлайн, колонку.",
    inputSchema: z.object({
...
server.registerTool(
  ...
  async () => {
    const data = await apiv2("GET", "/users?limit=100") as { content?: Array<{ id: string; username: string; email: string }> };
    const users = data.content || [];
    if (!users.length) return text("Пользователи не найдены");
    const list = users.map(u => `• ${u.username} (${u.email}) [id: ${u.id}]`).join("\n");
...
server.registerTool(
  "yougile_send_message",
  {
    ...
    description: "Отправить сообщение в чат задачи YouGile. Полезно для комментариев и обновлений статуса.",
    inputSchema: z.object({
  ...
  async ({ taskId, text: msgText }) => {
    const data = await apiv1("POST", "/messages", { taskId, text: msgText }) as { result: string; error?: string };
    if (data.result === "ok") return text(`✓ Сообщение отправлено в задачу ${taskId}`);
    return text(`Ошибка: ${data.error || JSON.stringify(data)}`);
  }
);
...
async function main(): Promise<void> {
  ...
  app.get("/", (_req, res) => {
    res.json({ status: "ok", service: "YouGile MCP Server", version: "1.0.0" });
  });
  ...
  app.listen(PORT, "0.0.0.0", () => {
    console.log(`YouGile MCP сервер запущен на порту ${PORT}`);
  });
}
